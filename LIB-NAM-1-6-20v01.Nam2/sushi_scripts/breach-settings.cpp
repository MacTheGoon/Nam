...
class BREACH 
	{	
		...
		gunClasses[] = {"rhs_weap_M590_5RD","CSW_M870_Sawed_Off"}; //< classes of guns that allowed to breach doors. RHS shotgun default.
		...
		class lockDoor {
			...
			{
				zone = "LockAllDoors"; //< marker (rectangle or elipse) that covers zone.  Change this name to the one you made
				closeDoors = 1; //<1- close all doors in zone, 0- do nothing
				lockAll = 3; //< 1- lock all closed doors in zone, 2-lock random (25%) closed doors, 3-lock 50% of doors in the zone, 4-lock 75% of doors in the zone
			};
		};
	};
